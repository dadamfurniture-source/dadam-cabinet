// ═══════════════════════════════════════════════════════════════
// BOM Tool - 부품 목록(BOM) 생성 MCP 도구
// StructuredDesignData → Bill of Materials
// ═══════════════════════════════════════════════════════════════
...
registerTool(
  {
    name: 'generate_bom',
    description: '설계 데이터에서 부품 목록(BOM)을 생성합니다. 캐비닛별 판재, 하드웨어, 설비, 부자재 등의 수량과 치수를 포함합니다.',
    inputSchema: {
      type: 'object',
      properties: {
        category: {
  ...
  async (args) => {
    ...
    try {
      ...
      return mcpError(`BOM generation failed: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
);
