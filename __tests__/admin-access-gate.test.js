...
function fakeClient({ session = { user: { id: 'u1' } }, rpc = { data: true, error: null }, calls = {} } = {}) {
  calls.rpc = 0;
  return {
    auth: { getSession: async () => ({ data: { session } }) },
    rpc: async () => { calls.rpc++; return rpc; },
    from: () => ({ select: () => ({ eq: () => ({ single: async () => ({ data: null, error: { message: 'x' } }) }) }) }),
  };
}
...
describe('isAdmin — 거짓을 캐시하지 않는다', () => {
  ...
  test('RPC 가 false 여도 캐시에 남기지 않는다 — 다음에 다시 묻는다', async () => {
    ...
    const c = {
      ...
      rpc: async () => { calls.rpc = (calls.rpc || 0) + 1; return answer; },
    };
    expect(await win.AdminAccess.isAdmin(c)).toBe(false);
  ...
  test('소스에 부정 캐시가 남지 않았다', () => {
    const fn = ADMIN.slice(ADMIN.indexOf('async function isAdmin'), ADMIN.indexOf('async function getRole'));
...
describe('관리자는 상세설계 승인으로 본다', () => {
  ...
  test('관리자가 아니면 profiles 를 본다', async () => {
    ...
    const c = {
      ...
      from: () => ({ select: () => ({ eq: () => ({ single: async () => ({
        data: { detaildesign_approved: false, detaildesign_requested_at: '2026-08-26' }, error: null,
      }) }) }) }),
    };
    const r = await win.DetailDesignAccess.fetchStatus(c, 'u1');
  ...
  test('미승인은 캐시하지 않는다 — 승인 직후 바로 열린다', async () => {
    ...
    const c = {
      ...
      rpc: async () => ({ data: false, error: null }),
      from: () => ({ select: () => ({ eq: () => ({ single: async () => ({
        data: { detaildesign_approved: approved, detaildesign_requested_at: null }, error: null,
      }) }) }) }),
    };
...
describe('차단 게이트 두 곳 모두 관리자를 통과시킨다', () => {
  test('detaildesign.html — isAdmin 이면 즉시 return', () => {
    ...
    const guard = html.slice(html.indexOf('enforceDetailDesignAccess'), html.indexOf('async function requestDetailDesignAccess'));
  ...
  test('그 밖의 페이지는 차단이 아니라 메뉴 노출만 다룬다', () => {
    ['ai-design.html', 'collection.html', 'material.html', 'my-designs.html', 'index.html', 'consultation.html']
      .forEach((f) => {
        const src = read(f);
        expect(src).toContain('AdminAccess.updateNavLinks');
