...
describe('모듈 규약', () => {
  ...
  test('아직 어느 페이지도 싣지 않는다 (R1 은 되살리기만 한다)', () => {
    // 2026-09-19: 이 파일은 7b99f21 에서 사진 모드와 함께 지워졌다가 되살아났다.
    //   지운 이유는 수학이 틀려서가 아니라 **사람이 사각형을 끄는 UI** 때문이었다
    //   (docs/01-plan/photo-composite-research.plan.md §4). 수학만 돌아오고 UI 는 안 온다.
    //   화면에 붙이는 것은 R4 다 — 그때 이 시험을 '구조 페이지가 싣는다' 로 바꾼다.
    for (const page of ['mockup-structure.html', 'mockup-shell.html']) {
      expect(fs.readFileSync(path.join(ROOT, page), 'utf8')).not.toContain('photo-solve.js');
    }
    // 되살리면 안 되는 짝들 — 이것들이 돌아오면 접었던 수작업 UI 가 같이 돌아온 것이다
    for (const gone of ['js/planner/photo-mode.js', 'js/planner/photo-bg.js']) {
