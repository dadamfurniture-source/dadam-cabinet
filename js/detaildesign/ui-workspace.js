      ...
      function openSpecPopup(itemUniqueId, section) {
        ...
        const contents = {
          dimensions: `
            ...
            <div class="spec-row"><div class="spec-field"><label>몸통 두께 (PB)</label><select onchange="updateSpecValue(${uid},'bodyThickness',this.value)">${(typeof BODY_THICKNESS_OPTIONS !== 'undefined' ? BODY_THICKNESS_OPTIONS : [15, 18]).map(t => `<option value="${t}" ${(parseFloat(item.specs.bodyThickness) || BODY_THICKNESS_DEFAULT) == t ? 'selected' : ''}>${t}T</option>`).join('')}</select></div><div class="spec-field"><label style="font-size:9px;color:#888;line-height:1.3;">천판·지판·선반 폭이 W-2T 로 따라갑니다 (15T→W-30, 18T→W-36). 도어 MDF 18T 는 무관.</label></div></div>`,
          hardware: `
