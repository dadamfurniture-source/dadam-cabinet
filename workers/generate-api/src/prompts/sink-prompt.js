...
[LOWER] 5 lower cabinets (600mm, sink, 600mm, cooktop, 600mm).
  - Below cooktop: a SOLID DRAWER STACK of EXACTLY 2 horizontal drawer fronts — a TWO-TIER drawer bank, one upper drawer face and one lower drawer face, together filling the ENTIRE cabinet height from directly under the countertop down to the toe kick. Count them: 2, not 1, not 3. The two faces split the module height roughly in half, are the same width as the module, and are separated by a single thin horizontal reveal line. The whole module must be completely covered by these two drawer faces — NO empty appliance slot, NO oven cavity, NO microwave cavity, NO dishwasher cavity, NO door, NO single tall drawer, NO third drawer. Each drawer front is a flat horizontal panel; the stack is uniform, fully closed, and shares the cabinet color. The cooktop module must look identical to a regular 2-tier kitchen drawer bank — only these two horizontal drawer fronts, nothing else.
...
[HANDLES] RECESSED GROOVE HANDLE ONLY — the single most important hardware rule; every panel in the image must obey it. Each lower panel has a 30mm horizontal channel groove recessed into its TOP edge, running the FULL width of the panel; each upper panel has the same 30mm groove recessed into its BOTTOM edge (mirrored). The groove is cut into the panel itself, finished in the exact cabinet color and material, and reads as a clean horizontal shadow line — identical width and depth on EVERY door and EVERY drawer front without exception, including both drawer faces of the cooktop module. A hand opens the panel by slipping into this recess; that groove is the ONLY grip anywhere in the kitchen. FORBIDDEN on every panel: knobs, pulls, bar handles, D-handles, chrome or metal or brass hardware, edge-mounted strips, surface-mounted trims, push-to-open buttons, touch latches, visible hinges, and any mixture of handle styles. If a panel would otherwise receive a handle, give it the recessed groove instead.
...
- Countertop: ${altCT}
- ${styleName} style
KEEP everything else PIXEL-IDENTICAL: camera angle, room background, cabinet layout, sink position, cooktop position, hood, door lines, appliance positions, lighting, AND the recessed groove handle style (uniform 30mm horizontal groove cut into the TOP edge of EVERY lower door/drawer and the BOTTOM edge of EVERY upper door — same width and depth on every single panel, finished in the new cabinet color; absolutely NO knobs, pulls, bar/D-handles, metal hardware, push-to-open buttons or visible hinges anywhere) AND the drawer stack below the cooktop (EXACTLY 2 horizontal drawer fronts — a two-tier bank filling the full module height, NO third drawer, NO oven/microwave/dishwasher cavity, NO empty appliance slot) AND the undermount sink + matte black faucet + flush induction cooktop hardware.
