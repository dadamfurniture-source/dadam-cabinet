...
`_convertPlannerModules` → `MaterialExtractor.extract`)로 흘려 (플래너 모듈, 부재 종류) 348 묶음을 대조하니
**53 묶음만 맞고 295 건이 어긋난다.** 측판·표준 하부장 도어·멍장 목대·키큰장 하부 단 도어는 이미 같다.
(처음 보고는 362 묶음 · 309 건이었다. 2026-09-15 "분배기는 장이 아니다" 결정으로 #8 가전 영역 모듈 14 건이 사라졌다 — §4 P4-11.)
어긋남은 원인 14 묶음으로 나뉘고, 그중 **키큰장 단이 하부장 규칙을 타는 것**(도어 H−30 · 선반 강제 · 걸레받이
...
295 건은 허용 목록에 이유와 함께 **전부** 기록됐고, 시험은 새 차이도 낡은 항목도 막는다 — 항목은 코드를 고쳐야만 줄어든다.
...
- `three.moduleGroup` 을 순회해 Mesh 만 센다. `PLANNER_FINISH_PAINT_SKIP` (`edge` `reveal` `doorEdge` `carcass-line`
  `pick` `area` `module` `leg` `marker`) 는 표시용이거나 철물(다리발)이라 뺀다. `marker` 는 가전 자리 표시(분배기·후드)의
  반투명 상자다 — 장이 아니라 부재도 아니다 (2026-09-15, `PLANNER_MARKER_SECTIONS`). `open` 칸의 와이어프레임 틀은
...
|---|---|---|---|---|---|---|---|
| straight | 10 | 80 (99) | 73 (107) | 98 | 14 | 84 | 25 / 18 / 41 |
| lShape | 11 | 87 (106) | 78 (112) | 108 | 16 | 92 | 30 / 21 / 41 |
| oblique | 6 | 55 (68) | 50 (74) | 68 | 10 | 58 | 18 / 13 / 27 |
| cornerL | 6 | 60 (70) | 52 (73) | 74 | 13 | 61 | 22 / 14 / 25 |
| **합** | 33 | 282 (343) | 253 (366) | **348** | **53** | **295** | 95 / 66 / 134 |

(2026-09-15 갱신 — 분배기 표시 모듈 `sink-0` 의 3D 부재 7 종(9 판)이 straight·lShape 에서 빠졌다. 모듈 수는 그대로다:
표시 모듈은 남고 장만 안 세운다.)
...
295 건, 원인 14 묶음 (처음 15 — #8 은 2026-09-15 에 해결돼 0 건). 판정: **BOM 버그**(발주가 틀린다) · **3D 오류**(도면이 규칙과 다르다) · **단순화**(3D 가 일부러
...
| 8 | 가전 영역 모듈 | ~~14~~ → 0 | only3d | (해결) 분배기(`sink`) 배치 공간에 자동계산이 장(700×500×370)을 세워 3D 가 몸통·도어·상판을 그렸다. 브리지는 sink 를 가전으로 보고 BOM 에서 뺐다 | **해결** — 2026-09-15 결정, P4-11 |
...
11. ~~**분배기 영역의 장** (#8)~~ — **해결 (2026-09-15 사용자 결정).** "'분배기'는 씽크볼이 앉는 자리 표시일 뿐 장이 아니다.
    개수대 하부장은 하부 라인 안의 셀(브리지가 `개수대` 로 라벨)이다." 그래서 BOM 이 맞고 3D 가 틀렸다:
    - 정본 `js/planner/planner-sections.js PLANNER_MARKER_SECTIONS = ['sink', 'hood']` — 후드도 같다(후드장은 상부 라인의 셀).
    - 전체 자동계산은 표시 공간에 **공간 폭 그대로의 표시 모듈 하나**만 둔다 (`autoCalcMarkerArea`). 모듈은 남아야
      브리지가 그 X 범위로 개수대·후드장 셀을 판정한다 (`PLANNER_APPLIANCE_SECTIONS`). 구조 `structures['sink-N']` 은
      만들지도 저장하지도 않는다 (`getStructure` 가 저장 없는 `markerStructure()` 를 준다; 예전 저장분은 열 때 버린다).
    - 3D 는 섹션 색 반투명 상자 + 윤곽선(`buildMarkerMesh`, entityKind `marker`), 정면도는 점선 사각형 + 이름
      (`renderMarkerFront`). 몸통·도어·상판·처짐방지목·다리발 없음.
    - 도어 최소폭 350 문턱은 그대로다 — 기본 후드 300 은 예전처럼 모듈이 서지 않는다 (규칙을 새로 만들지 않는다).
    - 불변조건 I1: 골든 payload 는 자동계산 전 스냅샷(`structures: {}`)이라 바이트 동일. 시험 `__tests__/planner-sink-marker.test.js`.
    - 식기세척기·냉장고는 이 결정 밖이다 — 가전을 감싸는 장이 실제로 있는지 따로 정한다.
