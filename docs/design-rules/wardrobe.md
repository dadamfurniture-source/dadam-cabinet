...
> **아직 코드가 표현하지 못하는 것.** 모듈 타입은 짧은옷·긴옷·선반형 셋뿐이라 3번 통의 중간 칸막이와
> 4번 통 하부의 옆 분할을 담을 자리가 없다. 붙박이장 모듈을 눌러 구조를 옵션으로 고르는 기능에서
> 같이 넣는다 (2026-09-16 지시). 그때까지 자동계산은 §9 의 4통 배치(짧은옷·짧은옷·긴옷·선반형)를 쓴다.
