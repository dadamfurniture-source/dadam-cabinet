...
```
사진(원본)  ─┐
             ├─ 같은 카메라 ─→ 가구 렌더(투명 배경 PNG) ─┐
바닥 사각형 ─┘                                          ├─ 2D 캔버스 합성 → PNG
             └─ 그림자 받개 렌더(그림자만, 알파) ────────┘
```

- 가구와 그림자를 **따로** 뽑는다. 그림자 세기·번짐을 합성 단계에서 조절할 수 있고,
  다시 렌더하지 않고 슬라이더로 즉시 바뀐다.
...
| `plane jsonb` | `{ kind:'floor', areaId, rectMm:{w,d}, originMm:{x,z}, rotation }` — 어느 배치 공간의 바닥인가 |
...
### P1. 배경 올리기 · 사각형 맞추기 (L) `agent/photo-background-ui`
- 우측 패널 새 섹션 `data-sec="photo"` (§1.1 규약대로 3곳 수정).
- 사진 올리기: 드래그·파일 선택. **EXIF 방향 정규화**(캔버스로 다시 그려 방향을 굽는다),
  긴 변 4096 로 축소, JPEG 재인코딩. `ai-design.html:480-512` 의 방식을 참고하되 별도 함수로 쓴다.
- 3D 캔버스 **뒤에** 사진 레이어(`<img>` 또는 CSS background). 캔버스는 `alpha:true` 라 비워 두면 비친다 (G1).
- 사진 위 사각형 편집기: 네 점 드래그, 스냅, 되돌리기. 격자를 겹쳐 그려 평면이 맞았는지 눈으로 확인.
- 실제 치수는 선택한 배치 공간에서 자동으로 받고 수정 가능 [확인 필요: 바닥 기준인지 벽 기준인지].
- 화각 슬라이더(퇴화일 때 자동으로 열림), 재투영 오차 표시.
- 완료: 사진 위에서 가구가 원근이 맞게 **화면에** 보인다. 저장은 아직 없다.

### P2. 그림자 · 빛 · 톤 (M) `agent/photo-shadow-light`
- 그림자 받개 평면 + `ShadowMaterial`, 세기·번짐 슬라이더.
- 빛 방향 다이얼(방위각·고도). **조명 위치 save/restore 를 자체적으로** 한다 (G6).
- 노출·색온도 슬라이더로 렌더를 사진 톤에 맞춘다. 사진의 평균 밝기를 읽어 초기값을 제안한다.
- 도어 테두리 끄기 (G7).
- 완료: 가구가 바닥에 붙어 보인다.
...
1. **사각형을 어디에 맞추나** — 가구가 설 **바닥**인가, 뒤 **벽면**인가. 바닥이 기하적으로 안정적이고
   그림자에도 유리하지만, 주방은 하부장이 바닥을 가려 귀퉁이가 안 보일 수 있다. 벽면 기준도 함께 지원할지.
2. **실제 치수의 출처** — 배치 공간 값을 그대로 쓰나, 사용자가 사진에서 잰 값을 넣나.
...
1. §9 의 1·2 답을 받는다 (나머지는 진행 중 받아도 된다).
2. `agent/photo-solve-core` (P0) — 수학과 왕복 시험. **UI 없이 정확도를 먼저 증명한다.**
3. 실제 주방 사진 3장을 기준 자료로 확보한다 (각도·밝기가 다른 것으로).
4. P0 이 통과하면 `agent/photo-background-ui` (P1) 로 화면에 붙인다.
