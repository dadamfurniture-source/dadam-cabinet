...
>
> 2026-09-18: **품질을 어떻게 올릴지는 `photo-composite-research.plan.md` 로 갈라 놓았다.**
>   이 문서는 무엇을 만드는가, 저 문서는 어떻게 좋게 만드는가다.
> 2026-09-19: 그 문서가 방향을 바꿨다 — **가구는 우리가 렌더해서 그대로 합성하고 AI 는 빛만 맞춘다.**
>   생성 모델은 형상을 보존하지 않는다는 것이 조사로 확인됐다. §1 의 '카메라 역산은 다시 만들지
>   않는다' 는 문장은 **사람이 사각형을 끄는 UI 에만** 해당한다 — 수학(`photo-solve.js`)은 되살린다.
>
