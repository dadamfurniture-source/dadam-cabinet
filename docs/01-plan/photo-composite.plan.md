...
- 사각형은 **가구가 설 바닥**에 맞춘다. 실제 치수는 **선택한 배치 공간에서 자동으로** 받는다
  (`planeBoxOf(area)` 의 W·D). 사람이 재서 넣지 않는다 — 도면이 이미 정답을 갖고 있고,
  손으로 넣으면 도면과 어긋난 값이 섞인다 (2026-09-17 결정).
...
1. `agent/photo-solve-core` (P0) — 수학과 왕복 시험. **UI 없이 정확도를 먼저 증명한다.**
2. 실제 주방 사진 3장을 기준 자료로 확보한다 (각도·밝기가 다른 것으로).
3. P0 이 통과하면 `agent/photo-background-ui` (P1) 로 화면에 붙인다.
