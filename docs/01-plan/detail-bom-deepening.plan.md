...
| 항목 | 규칙 소스 | 상태 |
|---|---|---|
| 상판 (PT18 / 인조대리석 12·50), 개수대·쿡탑 타공, 물끊기, 백스플래시 | `bom-protocol.md §2` 두께만 있음 | 규칙 **[확인 필요]** |
| 냉장고장 좌대·마감재·EP | fridge 분기가 생략 | `fridge.md` 로 명문화 |
| 경첩: 도어 폭·높이·오버레이별 개수·종류, EL 리프트업, 댐퍼 | `extractors.js:878` 높이만 | **[확인 필요]** |
| 레일: `drawerCount` 반영, 언더/사이드, 내부서랍 | `extractors.js:955` 과소 | 즉시 수정 |
| 체결구: 미니픽스·목다보·스크류·타카·본드 | `extractOthers` 빈 함수 | 규칙표 `docs/design-rules/hardware.md` 신설 **[확인 필요]** |
| 선반핀·걸레받이 클립·다리발 개수 | 부분 | 보강 |
...
8. **철물·체결구 규칙 값**: 경첩 개수표, 미니픽스/목다보/스크류 개수, 레일 종류.
9. **상판 규칙**: 두께·재질·타공·물끊기·이음 위치.
